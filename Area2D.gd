...
onready var nerd = get_parent().get_node("Nerd")
...
func _ready():
	...
	connect("area_entered", nerd, "colidiu")
	nerd.connect("recomecou", self, "recomecou")
	pass
...
func _physics_process(delta):
	...
	if not pulou:
		if get_position().x < main.get_node("Nerd").get_position().x:
			pulou = true
